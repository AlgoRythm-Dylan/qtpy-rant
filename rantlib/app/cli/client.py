...
from rantlib.app.cli.command.command import CommandInput, Command
from rantlib.app.cli.config import TerminalConfig
from rantlib.app.storage import STD_PATH_CLI_CONFIG
from rantlib.app.event.command import *
from rantlib.app.cli.prompt import Prompt
        ...
        for dirpath, dirnames, filenames in walk(commands_path):
            for file in filenames:
                if file.startswith("cmd_"):
                    self.import_module(f"rantlib.app.cli.command.{file[:file.find('.py')]}")
            break
